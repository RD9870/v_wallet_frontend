                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          IconButton(
                            onPressed: () async {
                              // نفتح شاشة الماسح
                              // الـ Provider هو من سيتولى عملية المسح والتحويل وإغلاق الشاشة عند النجاح
                              final result = await Navigator.of(context).push(
                                MaterialPageRoute(
                                  builder: (context) =>
                                      const QrScannerScreen(), // تأكد من استخدام اسم الكلاس الصحيح
                                ),
                              );

                              // إذا أردت تحديث البيانات (مثل الرصيد) في الصفحة الرئيسية بعد نجاح التحويل:
                              if (result == true) {
                                // هنا يمكنك استدعاء دالة تحديث الملف الشخصي أو الرصيد
                                // context.read<UserProvider>().refreshBalance();
                                debugPrint(
                                  "تم التحويل بنجاح، يمكنك تحديث واجهة الرصيد هنا",
                                );
                              }
                            },
                            icon: Icon(
                              Icons.qr_code,
                              color: whiteColor, // تأكد أن whiteColor معرف عندك
